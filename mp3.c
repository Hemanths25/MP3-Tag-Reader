...
//copying header to copy file
Status copy_header(mp3Info *MP3INFO){
    //rewinding both file src and copy file pointers
    rewind(MP3INFO->copy_pointer);
    rewind(MP3INFO->mp3_pointer);
    //copying 10 bytes header
    char header[10];
    fread(header,1,10,MP3INFO->mp3_pointer);
    fwrite(header,1,10,MP3INFO->copy_pointer);
}
...
//changing tag details
Status tag_edit(mp3Info *MP3INFO){
    //if particlular tag is not passed
...
Status edit(mp3Info *MP3INFO){
    ...
    if(copy_header(MP3INFO)==e_failure){
        return e_failure;
    }
